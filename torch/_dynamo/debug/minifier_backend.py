...
def fails_accuracy(fx_g, real_inputs, compiler_fn, compiled_fn, run_fn):
    if compiled_fn is None:
        compiled_fn = compile_from_real_inputs(fx_g, compiler_fn, real_inputs)
    ref = run_fwd_maybe_bwd(fx_g, real_inputs, True)
    res = run_fwd_maybe_bwd(compiled_fn, real_inputs, True)
    if same(ref, res):
        return RuntimeResult(res, False, None)
    else:
        return RuntimeResult(None, True, "Accuracy failed")
    ...
    def inner(gm, example_inputs, **kwargs):
        ...
        if config.repro_after == "dynamo":
            ...
            if config.repro_level in (1, 2, 3):
                ...
                check_runtime_fn = functools.partial(fails_accuracy, run_fn=run_fn)
